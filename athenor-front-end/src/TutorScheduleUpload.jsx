...
import { getScheduleAsArray } from './mockData';
...
export default function TutorScheduleUpload() {
  ...
  // Fetch all schedules on component mount (demo mode)
  useEffect(() => {
    const loadMockSchedules = () => {
      // Demo mode - load from mock data
      const data = getScheduleAsArray();
  ...
  const handleDeleteTutor = (tutorName) => {
    ...
    setModalMessage("Tutor schedule deleted (demo mode)!");
  ...
  const handleSaveSchedules = () => {
    ...
    console.log('Demo mode: Schedules saved locally', scheduleData);
    
    setModalTitle("Success");
    setModalMessage("All schedules saved (demo mode)!");
    setModalType("success");
    setModalOpen(true);
    setLoading(false);
