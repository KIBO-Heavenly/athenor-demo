...
import { MOCK_TUTOR_COLORS } from './mockData';
...
export default function WordDocumentUpload() {
  ...
  // Load custom tutor colors on mount (demo mode)
  React.useEffect(() => {
    loadTutorColors();
  }, []);

  const loadTutorColors = () => {
    // Demo mode - use mock tutor colors
  ...
  const handleSaveToDatabase = () => {
    ...
    const existingNames = existingLocalData.map(oa => oa.tutorName.toLowerCase().trim());
    ...
    setModalTitle("Success");
    setModalMessage(`Successfully saved ${newTutors.length} user(s) (demo mode)!`);
    setModalType("success");
    setModalOpen(true);
